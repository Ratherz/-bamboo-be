                        ...
                        @foreach ($products as $item)
                            @if ($item->user_id == Auth::user()->id)
                                <tr>
                                    ...
                                    <td>
                                        @foreach ($productImg as $item2)
                                            {{-- {{ dd($item->id) }}
                                            --}}
                                            @if ($item2['user_id'] == Auth::user()->id && $item2['product'] == $item->id)
                                                <img src="{{ url('public/storage/') . '/' . $item2['path'] }}"
                                                    style="height: 35px;width:auto;object-fit: cover;">
                                            @endif
